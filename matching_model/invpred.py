...
# 투자사 번호 리스트 (0부터 300까지) - 각 번호는 특정 투자사를 나타냄
...
# pre-training 한 모델 불러오기
# model_type: 투자사 유형 예측 모델
model_type = torch.load('./SUIT_v1.01/models/model_type.pt')
# model_amount: 투자 금액 예측 모델
model_amount = torch.load('./SUIT_v1.01/models/model_amount.pt')
# model_field: 투자 분야 예측 모델
model_field = torch.load('./SUIT_v1.01/models/model_field.pt')
model_name = keras.models.load_model(
    './SUIT_v1.01/models/inv_name.h5')  # model_name: 투자사 이름 예측 모델


def inv_ftr(stup_ftr):
    """
    Level 1: 스타트업 특성을 기반으로 투자사 특성 예측

    Args:
        stup_ftr: 스타트업 특성 데이터 (투자단계, 자산, 부채, 고용인원 등)

    Returns:
        df_new: 예측된 투자사 특성이 추가된 데이터프레임
    """
    # --------------------------------1st layer--------------------------------#
    ...
    stup_ftr = stup_ftr.drop(columns=['자산', '부채', '스타트업 고용인원'])

    x1 = stup_ftr[['투자단계', 'log_자산', 'log_부채', 'log_스타트업 고용인원', '영업이익']].copy()
    x2 = stup_ftr[['ICT제조', '기타', '바이오/의료', '영상/공연/음반', '투자단계', 'log_자산', 'log_부채',
                   'log_스타트업 고용인원', '스타트업 업력구간', '자본', '영업이익', '당기순이익', '스타트업 소재지역 도']].copy()
    x3 = stup_ftr.copy()

    x1 = torch.Tensor(x1.values)
    x2 = torch.Tensor(x2.values)
    x3 = torch.Tensor(x3.values)

    # 투자사 유형 예측 (10개 유형 분류)
    new_type = model_type(x1)
    new_type = new_type.round()
    numpy_type = new_type.detach().numpy()
    df_type = pd.DataFrame(data=numpy_type, columns=[
                           "투자사 유형_1", "투자사 유형_2", "투자사 유형_3", "투자사 유형_4", "투자사 유형_5", "투자사 유형_6", "투자사 유형_7", "투자사 유형_8", "투자사 유형_9", "투자사 유형_10"])

    # 투자사 주요 투자 분야 예측 (6개 분야 분류)
    new_field = model_field(x2)
    new_field = new_field.round()
    numpy_field = new_field.detach().numpy()
    df_field = pd.DataFrame(data=numpy_field, columns=[
                            'ICT서비스', 'ICT제조', '기타', '바이오/의료', '영상/공연/음반', '유통/서비스'])

    # 투자 금액 예측 (연속값 회귀)
    new_amount = model_amount(x3)
    numpy_amount = new_amount.detach().numpy()
    new_amount = np.expm1(numpy_amount)
    df_amount = pd.DataFrame(data=numpy_amount, columns=['투자유치금액'])

    with ExcelWriter('SUIT_inv_ftr_prediction.xlsx') as writer:
        df_type.to_excel(writer, sheet_name='예측된 투자사 유형', index=False)
        df_field.to_excel(writer, sheet_name='예측된 투자사 주요투자분야', index=False)
        df_amount.to_excel(writer, sheet_name='예측된 투자유치금액', index=False)

    df_inv = pd.read_csv('./SUIT_v1.01/inv_v1.01.csv')
    df_new = pd.concat([df_inv, df_type, df_field, df_amount], axis=1)
...
def inv_name(df_new):
    """
    Level 2: 예측된 투자사 특성을 기반으로 매칭 가능성이 높은 투자사 이름 추천

    Args:
        df_new: Level 1에서 예측된 투자사 특성이 포함된 데이터프레임
...
# --------------------------------2nd layer--------------------------------#
    ...
    X = df_new.iloc[:, 1:].copy()  # 모델 입력 특성 준비 (첫 번째 열은 인덱스이므로 제외)
    y_pred_prob = model_name.predict(X)  # 투자사 이름 예측 (301개 투자사에 대한 확률 예측)
    top_20_indices = np.argsort(y_pred_prob, axis=1)[
        :, -20:]  # 확률이 높은 상위 20개 투자사 인덱스 추출
